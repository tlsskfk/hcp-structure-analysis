...
# library(readxl)
# RAVLTX2min = read_excel("../data//ms_99_LVL2__fit_totalcog_4-20-24.xlsx")
# RAVLTX2min = read.csv("../pvalue/ms_99_LVL2_2minxtotcog.csv")
...
RAVLTX2min = pmt[,c(1,2,4)]
roi_RAVLTX2min = RAVLTX2min$ROI
idx = d$regionlvl2b %in% roi_RAVLTX2min
d_RAVLTX2min = d[idx,]
roi2 = unique(RAVLTX2min$ROI)
...
resls_r2 = matrix(list(), 10,2)
for (i in 1:length(roi2)) {
  print(sprintf("processing %s", roi2[i]))
  resls_r2[[i,1]] = roi2[i]
  d1 = d_RAVLTX2min[d_RAVLTX2min$regionlvl2b == roi2[i],]
  t = d1%>%
    group_by(ID) %>%
    summarise(mean_greymatter = mean(graymatter))
  dd = merge(t,dt)
  resls_r2[[i,2]] = get_result_r2(dd)
}
...
out_r2 = merge(RAVLTX2min, out, by = "ROI")
out_r2 = out_r2[order(out_r2$p_2min2, decreasing = T),]
write.csv(out_r2, "./out_r2.csv")
