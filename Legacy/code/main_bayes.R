...
md = "graymatter~ scale(Total_cog) + scale(Age_yrs)  + Gender + scale(BMI) + scale(walk_pace) + scale(Education_yrs) +  scale(EstimatedTotalIntraCranialVol) + \
                          (1 + scale(Total_cog) + scale(Age_yrs)  + Gender + BMI + scale(walk_pace) + scale(Education_yrs) +  scale(EstimatedTotalIntraCranialVol) | regionlvl2b/regionlvl1b) + \
                          (1 | ID)"
temp = run_model_prior(md, data = data_50)
result = run_model(temp, cores = 10, threads = threading(2), file = './result_R10_50.rds')
